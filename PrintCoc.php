...
if (isset($_GET['TransNo'])) {
	$InvOrCredit = $_GET['TransNo'];
} elseif (isset($_POST['InvOrCredit'])) {
	$InvOrCredit = $_POST['TransNo'];
}
...
if (isset($PrintPDF) AND isset($TransNo)){
	...
	$UserLanguage = $_SESSION['Language'];
		...
		if (DB_error_no()!=0) {
			...
			prnMsg( _('There was a problem retrieving the CoC data for') . ' ' . $TransNo . ' ' . _('from the database') . '. ' . _('To print an invoice, the sales order record, the customer transaction record and the branch record for the customer must not have been purged') . '. ' . _('To print a credit note only requires the customer, transaction, salesman and branch records be available'),'error');
			if ($debug==1) {
